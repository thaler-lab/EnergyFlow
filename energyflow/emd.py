...
# emd(ev0, ev1, dists=None, R=1.0, beta=1.0, norm=False, gdim=None, dtype='float64',
#                periodic_phi=False, mask=False, return_flow=False,
#                n_iter_max=100000,
#                epsilon_large_factor=10000.0, epsilon_small_factor=1.0)
def emd(ev0, ev1, dists=None, R=1.0, beta=1.0, norm=False, gdim=None, dtype='float64',
                  periodic_phi=False, mask=False, return_flow=False,
                  n_iter_max=100000,
                  epsilon_large_factor=10000.0, epsilon_small_factor=1.0,
                  **kwargs):
    ...
    **Returns**
    ...
    kwargs_check('emd', kwargs, old_kwargs)
    for k in kwargs:
        if k == 'phi_col' and kwargs[k] != 2:
            raise ValueError('`phi_col` should only be 2')
        warnings.warn("Keyword argument '{}' has no effect on `emd`.".format(k)
                      + " Use `emd_pot` if you need previous functionality.")

    # get EMD object with proper options (global in case we want to inspect it later)
    global EMD
    if periodic_phi:
        EMD = wasserstein.EMDYPhi(R, beta, norm, dtype=dtype)
    else:
        EMD = wasserstein.EMD(R, beta, norm, dtype=dtype)
    EMD.set_network_simplex_params(n_iter_max, epsilon_large_factor, epsilon_small_factor)

    # run using euclidean distances
    ev0, ev1 = np.asarray(ev0), np.asarray(ev1)
    if dists is None:
        if ev0.ndim != 2 or ev1.ndim != 2:
            raise ValueError('event must be a 2-dimensional array')
        if gdim is not None:
            if ev0.shape[1] <= gdim or ev1.shape[1] <= gdim:
                raise ValueError('event should have at least gdim={} coordinates'.format(gdim))
            ev0, ev1 = ev0[:,:gdim+1], ev1[:,:gdim+1]
        ...
        # evaluate EMD
        emd = EMD(ev0[:,0], ev0[:,1:], ev1[:,0], ev1[:,1:])
    ...
    # run using custom distances
        ...
        # evaluate EMD
        emd = EMD(ev0, ev1, dists)

    # get flows if requested
    if return_flow:
        return emd, EMD.flows()
    else:
        return emd

# emds(events0, events1=None, R=1.0, beta=1.0, norm=False, gdim=None, dtype='float64',
#.                            pairwise_emd=None,
#                             periodic_phi=False, mask=False,
#                             external_emd_handler=None,
#                             events0_weights=None, events1_weights=None,
...
def emds(events0, events1=None, R=1.0, beta=1.0, norm=False, gdim=None, dtype='float64',
                                pairwise_emd=None,
                                periodic_phi=False, mask=False,
                                external_emd_handler=None,
                                events0_weights=None, events1_weights=None,
                                ...
                                epsilon_large_factor=10000.0, epsilon_small_factor=1.0,
                                **kwargs):
    r"""Compute the EMDs between collections of events using the Wasserstein
    ...
    **Returns**
    ...
    # warn about old kwargs
    old_kwargs = {'X0', 'X1', 'measure', 'coords', 'phi_col', 'empty_policy'}
    kwargs_check('emds', kwargs, old_kwargs)
    for k in kwargs:
        if k == 'phi_col' and kwargs[k] != 2:
            raise ValueError('`phi_col` should only be 2')
    ...
    # use provided object or create one if needed
    if pairwise_emd is not None:
        PairwiseEMD = pairwise_emd
    else:
        if periodic_phi:
            PairwiseEMD = wasserstein.PairwiseEMDYPhi(R, beta, norm,
                                                      n_jobs, print_every, bool(verbose),
                                                      throw_on_error=throw_on_error)
        else:
            PairwiseEMD = wasserstein.PairwiseEMD(R, beta, norm,
                                                  n_jobs, print_every, bool(verbose),
                                                  throw_on_error=throw_on_error)
        PairwiseEMD.set_network_simplex_params(n_iter_max, epsilon_large_factor, epsilon_small_factor)
        globals()['PairwiseEMD'] = PairwiseEMD

    # set handler if given
    if external_emd_handler is not None:
        PairwiseEMD.set_external_emd_handler(external_emd_handler)

    if verbose > 0:
        print(PairwiseEMD)

    # run computation
    PairwiseEMD(events0, events1,
                event_weightsA=events0_weights, event_weightsB=events1_weights,
                gdim=gdim, mask=mask)

    # return flows if handler not provided
    if external_emd_handler is None:
        return PairwiseEMD.emds()
...
def _emd(ev0, ev1, R, no_norm, beta, euclidean, n_iter_max,
    ...
    _, cost, _, _, result_code = _emd_c(pTs0/rescale, pTs1/rescale, thetas, n_iter_max)
...
def emd_pot(ev0, ev1, R=1.0, norm=False, beta=1.0, measure='euclidean', coords='hadronic',
    ...
    **Returns**
    ...
    G, cost, _, _, result_code = _emd_c(pTs0/rescale, pTs1/rescale, thetas, n_iter_max)
    check_result(result_code)
