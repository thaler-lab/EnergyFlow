...
class CNN(NNBase):
    ...
    def _construct_model(self):
        ...
        for i,(filter_size, num_filter, pool_size, act, dropout, k_init, pad) in enumerate(conv_z):
            ...
            if pool_size > 0:
                self.model.add(keras.layers.MaxPooling2D(pool_size=pool_size, data_format=self.data_format, 
                                                            name=self._proc_name('max_pool_'+str(i))))

            # add dropout layer if we have a non-zero dropout rate
            if dropout > 0.:
                d_layer = keras.layers.SpatialDropout2D if i < self.num_spatial2d_dropout else keras.layers.Dropout
        ...
        self.model.add(keras.layers.Flatten(name=self._proc_name('flatten')))
        ...
        for i,(num_dense, act, dropout, k_init) in enumerate(dense_z):
            ...
            # add dense layer
            self.model.add(keras.layers.Dense(num_dense, kernel_initializer=k_init, name=self._proc_name('dense_'+str(i))))
            self._add_act(act)

            # add dropout layer if dropout is nonzero
            if dropout > 0.:
                self.model.add(keras.layers.Dropout(dropout, name=self._proc_name('dropout_'+str(i+num_dropout))))
